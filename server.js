...
try {
await pool.query(`CREATE TABLE IF NOT EXISTS citas ( uuid TEXT PRIMARY KEY, sede TEXT NOT NULL, sucursal_uuid TEXT, sucursal_nombre TEXT, agenda_uuid TEXT, agenda_descripcion TEXT, profesional_uuid TEXT, profesional_nombre TEXT, especialidad TEXT, cliente_uuid TEXT, cliente_nombre TEXT, cliente_apellido TEXT, cliente_rut TEXT, cliente_telefono TEXT, cliente_email TEXT, inicio TIMESTAMPTZ, fin TIMESTAMPTZ, estado_codigo TEXT, estado_descripcion TEXT, estado_pago TEXT, tratamientos JSONB, datos_completos JSONB, fecha_creacion TIMESTAMPTZ, actualizado_en TIMESTAMPTZ DEFAULT NOW() )`);
...
- Convierte fechas locales (Chile) a TIMESTAMPTZ para usar idx_citas_inicio.
  */
  const FILTRO_RANGO = `  inicio >= ($1::date AT TIME ZONE 'America/Santiago')   AND inicio < (($2::date + INTERVAL '1 day') AT TIME ZONE 'America/Santiago')   AND ($3 = 'ambas' OR sede = $3)`;

// ============================================
// METRICA 1: KPIs GENERALES
// ============================================
async function metricaKpis({ desde, hasta, sede }) {
const sql = `SELECT COUNT(*)::int AS total_citas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.CONFIRMADA}')::int AS confirmadas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.SUSPENDIDA}')::int AS suspendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.CANCELADA}')::int AS canceladas, COUNT(DISTINCT cliente_uuid)::int AS pacientes_unicos, COUNT(DISTINCT profesional_uuid)::int AS profesionales_activos, COUNT(DISTINCT especialidad)::int AS especialidades_activas FROM citas WHERE ${FILTRO_RANGO}`;
...
const sqlVentas = `SELECT COALESCE(SUM(monto), 0)::float AS ingresos_reales, COUNT(*)::int AS num_ventas FROM ventas WHERE fecha BETWEEN $1::date AND $2::date AND ($3 = 'ambas' OR sede = $3)`;
...
const sql = `SELECT profesional_uuid, profesional_nombre, COUNT(*)::int AS total, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show FROM citas WHERE ${FILTRO_RANGO} AND profesional_uuid IS NOT NULL GROUP BY profesional_uuid, profesional_nombre HAVING COUNT(*) >= 10 ORDER BY pct_no_show DESC NULLS LAST, total DESC LIMIT 30`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT cliente_uuid, TRIM(CONCAT(MAX(cliente_nombre), ' ', COALESCE(MAX(cliente_apellido), ''))) AS paciente, MAX(cliente_telefono) AS telefono, MAX(cliente_email) AS email, MAX(cliente_rut) AS rut, COUNT(*)::int AS total_citas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_shows, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show FROM citas WHERE ${FILTRO_RANGO} AND cliente_uuid IS NOT NULL GROUP BY cliente_uuid HAVING COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') >= 2 ORDER BY no_shows DESC, pct_no_show DESC LIMIT 100`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT cliente_uuid, TRIM(CONCAT(MAX(cliente_nombre), ' ', COALESCE(MAX(cliente_apellido), ''))) AS paciente, MAX(cliente_telefono) AS telefono, MAX(cliente_email) AS email, COUNT(*)::int AS total_citas, COUNT(*) FILTER (WHERE estado_codigo IN ('${ESTADO.SUSPENDIDA}', '${ESTADO.CANCELADA}'))::int AS suspensiones, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo IN ('${ESTADO.SUSPENDIDA}', '${ESTADO.CANCELADA}')) / NULLIF(COUNT(*), 0), 2)::float AS pct_suspension FROM citas WHERE ${FILTRO_RANGO} AND cliente_uuid IS NOT NULL GROUP BY cliente_uuid HAVING COUNT(*) FILTER (WHERE estado_codigo IN ('${ESTADO.SUSPENDIDA}', '${ESTADO.CANCELADA}')) >= 2 ORDER BY suspensiones DESC, pct_suspension DESC LIMIT 100`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT profesional_uuid, profesional_nombre, COUNT(*)::int AS total_citas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(DISTINCT cliente_uuid)::int AS pacientes_unicos, COUNT(DISTINCT especialidad)::int AS especialidades, (COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}') * ${TICKET_PROMEDIO})::bigint AS ingresos_estimados FROM citas WHERE ${FILTRO_RANGO} AND profesional_uuid IS NOT NULL GROUP BY profesional_uuid, profesional_nombre ORDER BY atendidas DESC, total_citas DESC LIMIT 30`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sqlActual = `SELECT especialidad, COUNT(*)::int AS citas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(DISTINCT cliente_uuid)::int AS pacientes FROM citas WHERE ${FILTRO_RANGO} AND especialidad IS NOT NULL GROUP BY especialidad ORDER BY citas DESC`;
const { rows: actual } = await pool.query(sqlActual, [desde, hasta, sede]);

const sqlAnterior = `SELECT especialidad, COUNT(*)::int AS citas FROM citas WHERE ${FILTRO_RANGO} AND especialidad IS NOT NULL GROUP BY especialidad`;
...
const sql = `SELECT EXTRACT(HOUR FROM inicio AT TIME ZONE 'America/Santiago')::int AS hora, COUNT(*)::int AS total, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show FROM citas WHERE ${FILTRO_RANGO} GROUP BY hora ORDER BY hora`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT EXTRACT(DOW FROM inicio AT TIME ZONE 'America/Santiago')::int AS dow, COUNT(*)::int AS total, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show FROM citas WHERE ${FILTRO_RANGO} GROUP BY dow ORDER BY dow`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
...
const sql = `WITH datos_paciente AS ( SELECT DISTINCT ON (cliente_uuid) cliente_uuid, cliente_nombre, cliente_apellido, cliente_telefono, cliente_email, cliente_rut, sede AS sede_principal FROM citas WHERE cliente_uuid IS NOT NULL AND ($1 = 'ambas' OR sede = $1) ORDER BY cliente_uuid, inicio DESC ), historia AS ( SELECT cliente_uuid, MAX(inicio) AS ultima_cita, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas_total, COUNT(*)::int AS total_citas FROM citas WHERE cliente_uuid IS NOT NULL AND ($1 = 'ambas' OR sede = $1) GROUP BY cliente_uuid ), con_futuro AS ( SELECT DISTINCT cliente_uuid FROM citas WHERE inicio > NOW() AND estado_codigo IN ('${ESTADO.CONFIRMADA}', '${ESTADO.ATENDIDA}') AND cliente_uuid IS NOT NULL ) SELECT h.cliente_uuid, TRIM(CONCAT(dp.cliente_nombre, ' ', COALESCE(dp.cliente_apellido, ''))) AS paciente, dp.cliente_telefono AS telefono, dp.cliente_email AS email, dp.cliente_rut AS rut, dp.sede_principal, h.ultima_cita, h.atendidas_total, h.total_citas, EXTRACT(DAY FROM NOW() - h.ultima_cita)::int AS dias_sin_volver FROM historia h JOIN datos_paciente dp USING (cliente_uuid) WHERE h.atendidas_total >= 1 AND h.ultima_cita < NOW() - ($2 || ' days')::interval AND h.cliente_uuid NOT IN (SELECT cliente_uuid FROM con_futuro) ORDER BY h.ultima_cita ASC LIMIT 300`;
...
const sql = `SELECT sede, COUNT(*)::int AS total_citas, COUNT(DISTINCT cliente_uuid)::int AS pacientes_unicos, COUNT(DISTINCT profesional_uuid)::int AS profesionales, COUNT(DISTINCT especialidad)::int AS especialidades, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(*) FILTER (WHERE estado_codigo IN ('${ESTADO.SUSPENDIDA}','${ESTADO.CANCELADA}'))::int AS suspensiones, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show, (COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}') * ${TICKET_PROMEDIO})::bigint AS ingresos_estimados FROM citas WHERE inicio >= ($1::date AT TIME ZONE 'America/Santiago') AND inicio < (($2::date + INTERVAL '1 day') AT TIME ZONE 'America/Santiago') GROUP BY sede ORDER BY total_citas DESC`;
...
const sql = `WITH demo AS ( SELECT DISTINCT ON (cliente_uuid) cliente_uuid, NULLIF(datos_completos->'cliente'->>'fecha_nacimiento', '') AS fnac_str, NULLIF(datos_completos->'cliente'->>'sexo', '') AS sexo FROM citas WHERE cliente_uuid IS NOT NULL AND ($1 = 'ambas' OR sede = $1) ORDER BY cliente_uuid, inicio DESC ), demo_calc AS ( SELECT cliente_uuid, sexo, CASE WHEN fnac_str ~ '^\\d{4}-\\d{2}-\\d{2}' THEN EXTRACT(YEAR FROM AGE(NOW(), fnac_str::date))::int ELSE NULL END AS edad FROM demo ) SELECT CASE WHEN edad IS NULL THEN 'sin_dato' WHEN edad < 18 THEN '0-17' WHEN edad < 30 THEN '18-29' WHEN edad < 45 THEN '30-44' WHEN edad < 60 THEN '45-59' ELSE '60+' END AS rango_edad, CASE WHEN sexo IS NULL THEN 'sin_dato' WHEN UPPER(sexo) IN ('M', 'MASCULINO', 'HOMBRE') THEN 'M' WHEN UPPER(sexo) IN ('F', 'FEMENINO', 'MUJER') THEN 'F' ELSE 'otro' END AS sexo_norm, COUNT(*)::int AS cantidad FROM demo_calc GROUP BY rango_edad, sexo_norm ORDER BY rango_edad, sexo_norm`;
...
const sql = `SELECT COALESCE( NULLIF(datos_completos->'cliente'->>'prevision', ''), NULLIF(datos_completos->'cliente'->'prevision'->>'descripcion', ''), NULLIF(datos_completos->'cliente'->'prevision'->>'nombre', ''), 'sin_dato' ) AS prevision, COUNT(*)::int AS total_citas, COUNT(DISTINCT cliente_uuid)::int AS pacientes, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show FROM citas WHERE ${FILTRO_RANGO} GROUP BY prevision ORDER BY total_citas DESC LIMIT 30`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT COALESCE( NULLIF(datos_completos->'origen'->>'descripcion', ''), NULLIF(datos_completos->'origen'->>'codigo', ''), NULLIF(datos_completos->>'origen', ''), NULLIF(datos_completos->>'canal', ''), 'sin_dato' ) AS origen, COUNT(*)::int AS total, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, ROUND(100.0 * COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}') / NULLIF(COUNT(*), 0), 2)::float AS pct_no_show FROM citas WHERE ${FILTRO_RANGO} GROUP BY origen ORDER BY total DESC LIMIT 20`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
...
const sql = `SELECT DATE(inicio AT TIME ZONE 'America/Santiago') AS dia, COUNT(*)::int AS total, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}')::int AS atendidas, COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.NO_SHOW}')::int AS no_show, COUNT(DISTINCT cliente_uuid)::int AS pacientes, (COUNT(*) FILTER (WHERE estado_codigo = '${ESTADO.ATENDIDA}') * ${TICKET_PROMEDIO})::bigint AS ingresos_estimados FROM citas WHERE ${FILTRO_RANGO} GROUP BY dia ORDER BY dia`;
const { rows } = await pool.query(sql, [desde, hasta, sede]);
return rows;
}
